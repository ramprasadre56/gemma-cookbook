"""
Extract Ornamental Musa Varieties - Single name format
Processes page 45 (0-indexed: 44) - only Musa section (bottom portion)
"""
import fitz
import json
import os
...
OUTPUT_DIR = "assets/heartyculture_catalogue/ornamental_musa_varieties"
PAGES = [44]
MIN_Y = 270  # Start at Musa section
MAX_Y = 650  # Before Pineapple section
...
def extract_page(doc, page_num, min_y=0, max_y=999):
    page = doc[page_num]
    
    skip_patterns = [
        r'^Heartyculture', r'^Kanha', r'^Call', 
        r'^Ornamental Musa Varieties$'
    ]
    
    # Extract images - only below min_y
    images = []
    for img in page.get_images(full=True):
        ...
        rect = rects[0]
        if rect.y0 < min_y or rect.y0 > max_y:
            continue
    ...
    # Extract text - single name per plant, only below min_y
    raw_texts = []
    ...
    name_rows = cluster_by_row(texts, threshold=80)
    sorted_names = []
...
def extract_musa():
    ...
    print("\nOrnamental Musa Varieties Extraction")
    print("=" * 60)
    ...
    for page_num in PAGES:
        ...
        images, names = extract_page(doc, page_num, MIN_Y, MAX_Y)
        ...
        for idx, name in enumerate(names):
            ...
            entry = {
                ...
                'category': 'Ornamental Musa Varieties',
                'page': page_num + 1,
            }
            ...
            if idx < len(images):
                ...
                entry['image'] = f"/heartyculture_catalogue/ornamental_musa_varieties/{filename}"
                print(f"      {name['name']}")
