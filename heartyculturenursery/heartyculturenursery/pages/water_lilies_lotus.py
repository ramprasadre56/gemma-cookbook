"""Water Lilies & Lotus Category Page"""
import reflex as rx
...
PLANT_CATEGORIES = [
    ...
    {"name": "Water Lilies & Lotus", "route": "/plants/water-lilies-lotus", "icon": "droplets", "active": True},
...
class WaterLiliesState(rx.State):
    plants: list[dict] = []
    search_query: str = ""
    
    def load_plants(self):
        try:
            json_path = Path("assets/heartyculture_catalogue/water_lilies_lotus/plants.json")
...
def sidebar_category_item(cat: dict) -> rx.Component:
    ...
    return rx.link(
        rx.hstack(
            rx.icon(cat["icon"], size=14, color="#3b82f6" if is_active else "#666"),
            rx.text(cat["name"], font_size="1em", font_weight="600" if is_active else "400", color="#3b82f6" if is_active else "#444", white_space="nowrap"),
            spacing="2", align="center", width="100%",
        ),
        href=cat["route"], text_decoration="none", display="block", padding="6px 8px", border_radius="8px",
        background="rgba(59, 130, 246, 0.1)" if is_active else "transparent",
        border_left="3px solid #3b82f6" if is_active else "3px solid transparent",
        transition="all 0.2s ease",
        _hover={"background": "rgba(59, 130, 246, 0.08)", "border_left": "3px solid #3b82f6"},
    )
    ...
    return rx.box(
        rx.vstack(
            ...
            rx.link(
                ...
                href="/plants", text_decoration="none", _hover={"color": "#3b82f6"},
            ),
...
rx.button(
        ...
        _hover={"box_shadow": "0 12px 28px rgba(0,0,0,0.12)", "transform": "translateY(-4px)", "border_color": "#3b82f6", "& .overlay-gradient": {"opacity": "1"}, "& img": {"transform": "scale(1.08)"}},
        transition="all 0.3s cubic-bezier(0.4, 0, 0.2, 1)", cursor="pointer",
    )


def water_lilies_lotus() -> rx.Component:
    return rx.box(
        navbar(),
        rx.box(
            rx.vstack(
                rx.hstack(
                    ...
                    rx.icon("chevron-right", size=16, color="#999"),
                    rx.text("Water Lilies & Lotus", color="#3b82f6", font_size="1em", font_weight="500"),
                    spacing="2", align="center",
                ),
                rx.hstack(
                    rx.box(rx.icon("droplets", size=24, color="white"), padding="12px", background="linear-gradient(135deg, #3b82f6, #2563eb)", border_radius="12px", box_shadow="0 4px 12px rgba(59, 130, 246, 0.3)"),
                    rx.vstack(
                        rx.heading("Water Lilies & Lotus", size="8", font_weight="700", color="#1a1a1a"),
                        rx.text(rx.text.span(WaterLiliesState.plant_count), rx.text.span(" varieties available"), color="#666", font_size="1.1em"),
            ...
            background="linear-gradient(180deg, #eff6ff, #ffffff)", width="100%",
        ),
        rx.hstack(
            category_sidebar(),
            rx.box(
                rx.box(
                    rx.hstack(
                        ...
                        rx.input(placeholder="Search water lilies...", value=WaterLiliesState.search_query, on_change=WaterLiliesState.set_search_query, border="none", outline="none", width="100%", font_size="1.1em", background="transparent", _focus={"outline": "none", "box_shadow": "none"}),
                        spacing="2", align="center", width="100%",
                    ),
                    ...
                    _focus_within={"border_color": "#3b82f6", "box_shadow": "0 0 0 3px rgba(59, 130, 246, 0.1)"},
                ),
                rx.box(
                    rx.foreach(WaterLiliesState.filtered_plants, lambda plant: plant_card(plant)),
