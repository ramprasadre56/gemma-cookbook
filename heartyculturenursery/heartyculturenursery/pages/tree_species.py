...
PLANT_CATEGORIES = [
    ...
    {"name": "Tree Species", "route": "/plants/tree-species", "icon": "trees", "active": True},
...
class TreeSpeciesState(rx.State):
    plants: list[dict] = []
    search_query: str = ""
    
    def load_plants(self):
        try:
            json_path = Path("assets/heartyculture_catalogue/tree_species/plants.json")
...
def sidebar_category_item(cat: dict) -> rx.Component:
    ...
    return rx.link(
        rx.hstack(
            rx.icon(cat["icon"], size=18, color="#15803d" if is_active else "#555"),
            rx.text(cat["name"], font_size="1em", font_weight="600" if is_active else "500", color="#15803d" if is_active else "#333"),
            spacing="3", align="center", width="100%",
        ),
        href=cat["route"], text_decoration="none", display="block", padding="6px 8px", border_radius="10px",
        background="rgba(21, 128, 61, 0.1)" if is_active else "transparent",
        border_left="4px solid #15803d" if is_active else "4px solid transparent",
        transition="all 0.2s ease",
        _hover={"background": "rgba(21, 128, 61, 0.08)", "border_left": "4px solid #15803d"},
    )
...
def category_sidebar() -> rx.Component:
    return rx.box(
        rx.vstack(
            ...
            rx.link(rx.hstack(rx.icon("chevron-left", size=16, color="#666"), rx.text("Plants", font_size="1em", color="#666"), spacing="1", align="center"), href="/plants", text_decoration="none", _hover={"color": "#15803d"}),
...
rx.button(
        ...
        _hover={"box_shadow": "0 12px 28px rgba(0,0,0,0.12)", "transform": "translateY(-4px)", "border_color": "#15803d", "& .overlay-gradient": {"opacity": "1"}, "& img": {"transform": "scale(1.08)"}},
        transition="all 0.3s cubic-bezier(0.4, 0, 0.2, 1)", cursor="pointer",
    )


def tree_species() -> rx.Component:
    return rx.box(
        navbar(),
        rx.box(
            rx.vstack(
                rx.hstack(rx.link("Home", href="/", color="#666", font_size="1em"), rx.icon("chevron-right", size=16, color="#999"), rx.link("Plants", href="/plants", color="#666", font_size="1em"), rx.icon("chevron-right", size=16, color="#999"), rx.text("Tree Species", color="#15803d", font_size="1em", font_weight="500"), spacing="2", align="center"),
                rx.hstack(
                    rx.box(rx.icon("trees", size=24, color="white"), padding="12px", background="linear-gradient(135deg, #15803d, #166534)", border_radius="12px", box_shadow="0 4px 12px rgba(21, 128, 61, 0.3)"),
                    rx.vstack(rx.heading("Tree Species", size="8", font_weight="700", color="#1a1a1a"), rx.text(rx.text.span(TreeSpeciesState.plant_count), rx.text.span(" varieties available"), color="#666", font_size="1.1em"), spacing="1", align_items="start"),
                    spacing="4", align="center",
                ),
                spacing="4", align_items="start", width="100%", padding="1.5em 2em",
            ),
            background="linear-gradient(180deg, #dcfce7, #ffffff)", width="100%",
        ),
        rx.hstack(
            category_sidebar(),
            rx.box(
                rx.box(
                    rx.hstack(rx.icon("search", size=20, color="#999"), rx.input(placeholder="Search tree species...", value=TreeSpeciesState.search_query, on_change=TreeSpeciesState.set_search_query, border="none", outline="none", width="100%", font_size="1.1em", background="transparent", _focus={"outline": "none", "box_shadow": "none"}), spacing="2", align="center", width="100%"),
                    background="white", border="1px solid #e0e0e0", border_radius="10px", padding="12px 16px", width="100%", max_width="350px", margin_bottom="1.5em", _focus_within={"border_color": "#15803d", "box_shadow": "0 0 0 3px rgba(21, 128, 61, 0.1)"},
                ),
                rx.box(rx.foreach(TreeSpeciesState.filtered_plants, lambda plant: plant_card(plant)), display="grid", grid_template_columns=rx.breakpoints(initial="repeat(2, 1fr)", sm="repeat(2, 1fr)", md="repeat(3, 1fr)", lg="repeat(3, 1fr)", xl="repeat(4, 1fr)"), gap="20px", width="100%"),
                flex="1", width="100%",
            ),
            spacing="4", align_items="start", width="100%", padding="1.5em 2em",
        ),
