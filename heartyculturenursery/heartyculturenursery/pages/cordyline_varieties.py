"""Cordyline Varieties Category Page"""
import reflex as rx
...
# All plant categories for sidebar navigation
PLANT_CATEGORIES = [
    ...
    {"name": "Cordyline Varieties", "route": "/plants/cordyline-varieties", "icon": "leaf", "active": True},
...
class CordylineState(rx.State):
    """State for the Cordyline Varieties page."""
    plants: list[dict] = []
    search_query: str = ""
    ...
    def load_plants(self):
        ...
        try:
            json_path = Path("assets/heartyculture_catalogue/cordyline_varieties/plants.json")
    ...
    def filtered_plants(self) -> list[dict]:
        ...
        return [
            ...
            if query in p.get("common_name", "").lower() 
            or query in p.get("scientific_name", "").lower()
        ]
...
def sidebar_category_item(cat: dict) -> rx.Component:
    ...
    return rx.link(
        rx.hstack(
            rx.icon(cat["icon"], size=14, color="#22c55e" if is_active else "#666"),
            rx.text(
                cat["name"],
                font_size="1em",
                font_weight="600" if is_active else "400",
                color="#22c55e" if is_active else "#444",
                white_space="nowrap",
            ),
            spacing="2",
            align="center",
        ...
        border_left="3px solid #22c55e" if is_active else "3px solid transparent",
        transition="all 0.2s ease",
        _hover={"background": "rgba(34, 197, 94, 0.08)", "border_left": "3px solid #22c55e"},
    )
...
def category_sidebar() -> rx.Component:
    ...
    return rx.box(
        rx.vstack(
            ...
            rx.link(
                rx.hstack(
                    rx.icon("chevron-left", size=16, color="#666"),
                    rx.text("Plants", font_size="1em", color="#666"),
                    spacing="1",
                    align="center",
                ),
                href="/plants",
            ...
            rx.vstack(
                *[sidebar_category_item(cat) for cat in PLANT_CATEGORIES],
                spacing="0",
                align_items="stretch",
                width="100%",
            ),
...
def cordyline_varieties() -> rx.Component:
    """Cordyline Varieties category page."""
    return rx.box(
        navbar(),
        # Header section
        rx.box(
            rx.vstack(
                rx.hstack(
                    ...
                    rx.icon("chevron-right", size=16, color="#999"),
                    rx.text("Cordyline Varieties", color="#f59e0b", font_size="1em", font_weight="500"),
                    spacing="2",
                    align="center",
                ),
                rx.hstack(
                    rx.box(
                        rx.icon("leaf", size=24, color="white"),
                        padding="12px",
                        background="linear-gradient(135deg, #f59e0b, #d97706)",
                        border_radius="12px",
                        box_shadow="0 4px 12px rgba(245, 158, 11, 0.3)",
                    ),
                    rx.vstack(
                        rx.heading("Cordyline Varieties", size="8", font_weight="700", color="#1a1a1a"),
                        rx.text(
                            rx.text.span(CordylineState.plant_count),
                            rx.text.span(" varieties available"),
                            color="#666",
                            font_size="1.1em",
                        ),
                        spacing="1",
            ...
            rx.box(
                rx.box(
                    rx.hstack(
                        ...
                        rx.input(
                            placeholder="Search cordyline varieties...",
                            value=CordylineState.search_query,
                            on_change=CordylineState.set_search_query,
                            border="none",
                            outline="none",
                            width="100%",
                            font_size="1.1em",
                    ...
                    max_width="350px",
                    margin_bottom="1.5em",
                    _focus_within={"border_color": "#f59e0b", "box_shadow": "0 0 0 3px rgba(245, 158, 11, 0.1)"},
                ),
                rx.box(
                    rx.foreach(CordylineState.filtered_plants, lambda plant: plant_card(plant)),
                    display="grid",
