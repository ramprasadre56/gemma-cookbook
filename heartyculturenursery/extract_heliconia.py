"""
Extract Heliconia Varieties - Handle both single and paired name formats
"""
import fitz
import json
import os
...
OUTPUT_DIR = "assets/heartyculture_catalogue/heliconia_varieties"
PAGES = [19, 20, 21]
...
def extract_page(doc, page_num):
    ...
    # Group into columns (wider threshold for Heliconia)
    columns = []
    for text in texts:
        found = False
        for col in columns:
            if abs(col['x'] - text['x']) < 80:
    ...
    # Extract plants - check if paired or single
    plants = []
    for col in columns:
        items = col['items']
        i = 0
        while i < len(items):
            ...
            # Check if there's a second line (common name)
            if i + 1 < len(items):
                t2 = items[i + 1]
                y_diff = t2['y'] - t1['y']
                
                # If close together (5-30px), it's a pair
                if 5 < y_diff < 30:
                    plants.append({
            ...
            # Single line entry - use the text as both sci/common name
            plants.append({
                'scientific_name': t1['text'],
                'common_name': t1['text'],
                'x': col['x'],
...
def extract_heliconia():
    doc = fitz.open(PDF_PATH)
    
    print("\nHeliconia Varieties Extraction")
    print("=" * 60)
    ...
    for page_num in PAGES:
        ...
        for idx, plant in enumerate(plants):
            ...
            if idx < len(images):
                ...
                entry['image'] = f"/heartyculture_catalogue/heliconia_varieties/{filename}"
