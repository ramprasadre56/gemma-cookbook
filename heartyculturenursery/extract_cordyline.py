"""
Extract Cordyline Varieties - Using same clustering logic as extract_draceana.py
Processes page 16 (0-indexed: 15) which contains Cordyline Varieties category
"""
import fitz
import json
import os
...
OUTPUT_DIR = "assets/heartyculture_catalogue/cordyline_varieties"
CORDYLINE_PAGE = 15  # 0-indexed
...
def extract_cordyline():
    doc = fitz.open(PDF_PATH)
    page = doc[CORDYLINE_PAGE]
    
    print(f"\n📄 Page {CORDYLINE_PAGE + 1}: Cordyline Varieties")
    print("=" * 60)
    ...
    skip_patterns = [r'^Heartyculture', r'^Kanha', r'^Call', r'^Cordyline Varieties$', r'^Cordyline varieties$']
    ...
    # Merge consecutive spans on same line (within 3px Y and 5px X gap)
    text_items = []
    i = 0
    ...
    for idx, plant in enumerate(sorted_plants):
        ...
        if idx < len(sorted_images):
            ...
            entry['image'] = f"/heartyculture_catalogue/cordyline_varieties/{filename}"
