"""
Extract Calathea Varieties - Single name format
Processes page 45 (0-indexed: 44) - only Calathea section (top portion)
"""
import fitz
import json
import os
...
OUTPUT_DIR = "assets/heartyculture_catalogue/calathea_varieties"
PAGES = [44]
MAX_Y = 300  # Only extract items above this Y position (exclude Ornamental Musa section)
...
def extract_page(doc, page_num, max_y=999):
    page = doc[page_num]
    
    skip_patterns = [
        r'^Heartyculture', r'^Kanha', r'^Call', 
        r'^Calathea Varieties$', r'^Ornamental'
    ]
    
    # Extract images - only above max_y
    images = []
    ...
    # Extract text - single name per plant, only above max_y
    raw_texts = []
...
def extract_calathea():
    doc = fitz.open(PDF_PATH)
    
    print("\nCalathea Varieties Extraction")
    print("=" * 60)
    ...
    for page_num in PAGES:
        ...
        images, names = extract_page(doc, page_num, MAX_Y)
        ...
        for idx, name in enumerate(names):
            ...
            if idx < len(images):
                ...
                entry['image'] = f"/heartyculture_catalogue/calathea_varieties/{filename}"
                print(f"      {name['name']}")
