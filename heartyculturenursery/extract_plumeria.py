"""
Extract Plumeria Varieties - Single name format (just variety names)
Processes pages 23-28 (0-indexed: 22-27)
"""
import fitz
import json
import os
...
OUTPUT_DIR = "assets/heartyculture_catalogue/plumeria_varieties"
PAGES = [22, 23, 24, 25, 26, 27]  # 0-indexed (pages 23-28)
...
def extract_page(doc, page_num):
    """Extract images and plant names from a single page"""
    page = doc[page_num]
    
    skip_patterns = [
        r'^Heartyculture', r'^Kanha', r'^Call', 
        r'^Plumeria Varieties$', r'^Plumeria varieties$'
    ]
    ...
    # Cluster names by row (same as images)
...
def extract_plumeria():
    doc = fitz.open(PDF_PATH)
    
    print("\nPlumeria Varieties Extraction")
    print("=" * 60)
    ...
    for page_num in PAGES:
        ...
        # Match by position - 1:1 matching
        for idx, name in enumerate(names):
            ...
            if idx < len(images):
                ...
                entry['image'] = f"/heartyculture_catalogue/plumeria_varieties/{filename}"
                print(f"      {name['name']}")
